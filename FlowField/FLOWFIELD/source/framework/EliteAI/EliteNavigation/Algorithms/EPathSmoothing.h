	...
	public:
		...
		static std::vector<Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int portalIdx = 1; portalIdx < amtPortals; ++portalIdx)
			{
				...
				if (Elite::Cross(newRightLeg, rightLeg) <= 0) // leg got smaller
				{
					if (Elite::Cross(newRightLeg, leftLeg) >= 0) // not crossing 
					{
				...
				if (Elite::Cross(newLeftLeg, leftLeg) >= 0) // leg got smaller
				{
					if (Elite::Cross(newLeftLeg, rightLeg) <= 0) // not crossing 
					{
						leftLeg = newLeftLeg;
						leftLegIdx = portalIdx;
					}
