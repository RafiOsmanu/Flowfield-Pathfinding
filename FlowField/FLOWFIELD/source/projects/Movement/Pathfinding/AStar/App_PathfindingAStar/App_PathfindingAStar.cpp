...
void App_PathfindingAStar::Update(float deltaTime)
{
		...
		&& startPathIdx != endPathIdx)
	{
		...
		auto heatMap = AStar<GridTerrainNode, GraphConnection>(m_pGridGraph, m_pHeuristicFunction);
		auto startNode = m_pGridGraph->GetNode(startPathIdx);
		auto endNode = m_pGridGraph->GetNode(endPathIdx);

		heatMap.FlowMap(startNode, endNode);
...
void App_PathfindingAStar::Render(float deltaTime) const
{
	...
	{
		//DEBUGRENDERER2D->DrawString(node.)
			//m_pGraphRenderer->
	}
...
#pragma region UI
	//UI
	{
		...
		ImGui::Begin("Flow Map", &windowActive, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
