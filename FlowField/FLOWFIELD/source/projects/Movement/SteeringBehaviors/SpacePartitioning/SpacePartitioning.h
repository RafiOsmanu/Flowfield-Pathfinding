...
public:
	...
	int RegisterNeighbors(SteeringAgent* agent, float queryRadius, std::vector<SteeringAgent*> &neighbours);
...
private:
	...
	Elite::Rect m_RadiusRect;
	SteeringAgent* m_RenderAgent = nullptr;
	int m_AmountOfAgents;
	// Helper functions
	int PositionToIndex(const Elite::Vector2 pos) const;
	int posToIndexConvert(const float pos, const float CellMeasure, const int nrOfCell ) const;
};
