...
{	
	...
	SAFE_DELETE(m_PrioritySteering);
	SAFE_DELETE(m_EvadingAgent);
	SAFE_DELETE(m_pFlee);
}
...
void App_CombinedSteering::Start()
{
	...
	m_PrioritySteering = new PrioritySteering({ m_pFlee, m_pSoberWander });

	m_EvadingAgent = new SteeringAgent();

	m_EvadingAgent->SetSteeringBehavior(m_PrioritySteering);
	m_EvadingAgent->SetMaxLinearSpeed(15.f);
	m_EvadingAgent->SetMass(0.f);
	m_EvadingAgent->SetAutoOrient(true);
	m_EvadingAgent->SetBodyColor({ 1, 0, 0 });
...
void App_CombinedSteering::Update(float deltaTime)
{
	...
	m_EvadingAgent->Update(deltaTime);
	m_EvadingAgent->SetRenderBehavior(m_CanDebugRender);


	if (m_TrimWorld)
	{
		m_pDrunkAgent->TrimToWorld(m_TrimWorldSize);
		m_EvadingAgent->TrimToWorld(m_TrimWorldSize);
	}
...
void App_CombinedSteering::Render(float deltaTime) const
{
	...
	m_EvadingAgent->Render(deltaTime);
